		...
		public IMessage onMessage(MessageLockSetLockCode message, MessageContext ctx) {
			...
			mainThread.addScheduledTask(() -> {
				...
				if (world.isBlockLoaded(message.pos)) {
					final ILock lock = LockUtils.getLock(world, message.pos, message.facing);
					if (lock != null) {
						if (lock.isLocked()) {
							player.addChatComponentMessage(new TextComponentTranslation("lockablecontainers:lock.already_locked"));
						}
